export interface ProjectHolidayDto { date:string; name:string|null }
export interface ProjectCalendarDto { timezone:"Asia/Seoul"; weekendDays:[6,0]; holidays:ProjectHolidayDto[] }
...
export interface ProjectMetadataMutationResponse { data:{project:ProjectDto;tasks:ProjectTaskDto[];links:ProjectLinkDto[];warnings:[];operation:{kind:"projectMetadata";changedFields:("name"|"description")[]}} }
export interface CreateTaskRequest { externalId?:string;parentTaskId?:string;convertParentToSummary?:true;name:string;type:"task"|"milestone";scheduleMode?:"auto"|"manual";start:string;end?:string;duration:number;progress:number;parentExternalId?:null;description?:string|null;url?:string|null }
export interface UpdateTaskRequest { name?:string;scheduleMode?:"auto"|"manual";start?:string;end?:string;duration?:number;progress?:number;description?:string|null;url?:string|null }
...
export interface TaskMutationResponse { data:{project:ProjectDto;tasks:ProjectTaskDto[];links:ProjectLinkDto[];warnings:ScheduleWarningDto[];operation:{kind:TaskMutationKind;changedTaskExternalIds:string[];deletedTaskExternalIds:string[];deletedLinkIds:string[]}} }
