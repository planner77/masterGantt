...
function canonicalUpdate(input: UpdateCatalogTargetRequest): UpdateCatalogTargetRequest {
  ...
  if (Object.keys(input).some((key) => !allowed.has(key)) || Object.keys(input).length === 0) {
    throw new ResourceCatalogInvalidInputError();
  }
  const result: UpdateCatalogTargetRequest = {};
  if (input.name !== undefined) {
    const name = normalizedText(input.name, 200);
    if (name === undefined) throw new ResourceCatalogInvalidInputError();
    result.name = name;
  }
  if (input.code !== undefined) {
    const code = normalizeCode(input.code);
    if (code === undefined) throw new ResourceCatalogInvalidInputError();
    result.code = code;
  }
  if (input.description !== undefined) {
    const description = normalizedText(input.description, 2000, true);
    if (description === undefined) throw new ResourceCatalogInvalidInputError();
    result.description = description;
  }
  if (input.active !== undefined) {
    if (typeof input.active !== "boolean") throw new ResourceCatalogInvalidInputError();
...
function sameTarget(current: CatalogTargetRecord, update: UpdateCatalogTargetRequest): boolean {
  return (update.name === undefined || update.name === current.name) &&
    (update.code === undefined || update.code === current.code) &&
...
export class ResourceCatalogService {
  ...
  constructor(private readonly database: Database.Database, options: ResourceCatalogServiceOptions = {}) {
    ...
    this.editSessions = new EditSessionRepository(database);
    this.schedules = new ScheduleRepository(database);
    this.clock = options.clock ?? (() => new Date());
    this.generatePublicId = options.generatePublicId ?? randomUUID;
    this.generateAssignmentPublicId = options.generateAssignmentPublicId ?? randomUUID;
  ...
  unlockAdmin(candidate: string, configuredPassword: string | undefined): { rawToken: string; expiresAt: string } | undefined {
    ...
    const expires = new Date(now.getTime() + RESOURCE_CATALOG_ADMIN_SESSION_TTL_SECONDS * 1000);
  ...
  authorizeAdmin(rawToken: string | undefined): { id: number; expiresAt: string } | undefined {
    ...
    const session = this.catalog.findAdminSessionByHash(hashSessionToken(rawToken));
  ...
  logoutAdmin(rawToken: string | undefined): void {
    const authorized = this.authorizeAdmin(rawToken);
    if (authorized) this.catalog.revokeAdminSession(authorized.id, this.clock().toISOString());
  }

  private requireAdmin(rawToken: string | undefined): void {
  ...
  private requireProjectAuthorization(authorization: AuthorizedEditSession): { id: number; revision: number } {
    ...
    const session = this.editSessions.findById(authorization.sessionId);
    if (!project || !session || project.publicId !== authorization.projectPublicId ||
      project.authVersion !== authorization.projectAuthVersion || session.projectId !== project.id ||
      session.authVersion !== project.authVersion || session.revokedAt !== null ||
      !session.tokenHash.equals(authorization.tokenHash) || Date.parse(session.expiresAt) <= this.clock().getTime()) {
      throw new ResourceCatalogAuthorizationError();
    }
  ...
  createTarget(kind: "resource" | "group", rawAdminToken: string | undefined, expectedRevision: number, input: CreateCatalogTargetRequest): ResourceCatalogResponse {
    ...
    const mutate = this.database.transaction(() => {
      ...
      if (!isCanonicalUuidV4(publicId)) throw new ResourceCatalogInvalidInputError();
      const now = this.clock().toISOString();
      if (kind === "resource") this.catalog.insertResource({ publicId, ...canonical, now });
      else this.catalog.insertGroup({ publicId, ...canonical, now });
  ...
  updateTarget(kind: "resource" | "group", publicId: string, rawAdminToken: string | undefined, expectedRevision: number, input: UpdateCatalogTargetRequest): ResourceCatalogResponse {
    ...
    const mutate = this.database.transaction(() => {
      ...
      if (sameTarget(current, canonical)) return this.getCatalog(rawAdminToken);
      const now = this.clock().toISOString();
      if (kind === "resource") this.catalog.updateResource(current.id, canonical, now);
      else this.catalog.updateGroup(current.id, canonical, now);
      if (!this.catalog.advanceRevision(expectedRevision, now)) throw new ResourceCatalogRevisionMismatchError();
      return this.getCatalog(rawAdminToken);
    });
    return mutate.immediate();
  ...
  replaceGroupMembers(groupPublicId: string, rawAdminToken: string | undefined, expectedRevision: number, input: ReplaceResourceGroupMembersRequest): ResourceCatalogResponse {
    ...
    const mutate = this.database.transaction(() => {
      this.requireAdmin(rawAdminToken);
      if (this.catalog.getRevision() !== expectedRevision) throw new ResourceCatalogRevisionMismatchError();
      const group = this.catalog.findGroupByPublicId(groupPublicId);
      if (!group) throw new ResourceCatalogTargetNotFoundError();
      const resources = input.resourceIds.map((id) => this.catalog.findResourceByPublicId(id));
      if (resources.some((resource) => !resource)) throw new ResourceCatalogTargetNotFoundError();
      const nextIds = [...input.resourceIds].sort();
      const currentIds = [...group.memberResourceIds].sort();
      if (nextIds.length === currentIds.length && nextIds.every((id, index) => id === currentIds[index])) return this.getCatalog(rawAdminToken);
      const now = this.clock().toISOString();
      this.catalog.replaceGroupMembers(group.id, resources.map((resource) => resource!.id), now);
      if (!this.catalog.advanceRevision(expectedRevision, now)) throw new ResourceCatalogRevisionMismatchError();
      return this.getCatalog(rawAdminToken);
    });
    return mutate.immediate();
  ...
  getAssignedTargets(projectPublicId: string): AssignedTargetsResponse | undefined {
    ...
    const assignments = this.catalog.listAssignments(project.id);
    const seen = new Set<string>();
    const targets: AssignmentTargetDto[] = [];
    for (const assignment of assignments) {
      const key = `${assignment.kind}:${assignment.targetPublicId}`;
      if (seen.has(key)) continue;
      seen.add(key);
      const target = assignment.kind === "resource" ? this.catalog.findResourceByPublicId(assignment.targetPublicId) : this.catalog.findGroupByPublicId(assignment.targetPublicId);
  ...
  replaceTaskAssignments(authorization: AuthorizedEditSession, expectedProjectRevision: number, taskPublicId: string, input: ReplaceTaskAssignmentsRequest): ReplaceTaskAssignmentsMutationResult {
    ...
    const mutate = this.database.transaction(() => {
      const currentProject = this.requireProjectAuthorization(authorization);
      if (currentProject.revision !== expectedProjectRevision) throw new ResourceCatalogProjectRevisionMismatchError();
      const catalogRevision = this.catalog.getRevision();
      if (catalogRevision !== input.catalogRevision) throw new ResourceCatalogRevisionMismatchError();
      const task = this.schedules.findTaskByPublicId(currentProject.id, taskPublicId);
      if (!task) throw new ResourceCatalogTaskNotFoundError();
      const resolved = input.targets.map((target) => {
        ...
        if (!record) throw new ResourceCatalogTargetNotFoundError();
        if (!record.active) throw new ResourceCatalogTargetInactiveError();
        return { kind: target.kind, publicId: target.id, internalId: record.id };
      });
      const current = this.catalog.listAssignments(currentProject.id).filter((assignment) => assignment.taskPublicId === taskPublicId);
      const currentKeys = current.map((assignment) => `${assignment.kind}:${assignment.targetPublicId}`).sort();
      const requestedKeys = [...keys].sort();
      if (currentKeys.length === requestedKeys.length && currentKeys.every((key, index) => key === requestedKeys[index])) {
