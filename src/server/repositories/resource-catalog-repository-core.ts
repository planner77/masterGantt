...
export class ResourceCatalogRepository {
  ...
  getRevision(): number {
    const row = this.database.prepare(
      "SELECT revision FROM resource_catalog_state WHERE id = 1",
  ...
  listResources(activeOnly = false): CatalogTargetRecord[] {
    const rows = this.database.prepare(
      `SELECT id, public_id, name, code, description, active
       FROM resources
       ${activeOnly ? "WHERE active = 1" : ""}
       ORDER BY lower(name), public_id`,
    ).all() as TargetRow[];
  ...
  listGroups(activeOnly = false): CatalogGroupRecord[] {
    ...
    const memberRows = this.database.prepare(
      `SELECT gm.group_id, r.public_id
       FROM resource_group_members gm
       JOIN resources r ON r.id = gm.resource_id
       ORDER BY gm.group_id, lower(r.name), r.public_id`,
    ).all() as { group_id: number; public_id: string }[];
  ...
  findResourceByPublicId(publicId: string): CatalogTargetRecord | undefined {
    const row = this.database.prepare(
      `SELECT id, public_id, name, code, description, active
       FROM resources WHERE public_id = ?`,
    ).get(publicId) as TargetRow | undefined;
  ...
  findGroupByPublicId(publicId: string): CatalogGroupRecord | undefined {
    const row = this.database.prepare(
      `SELECT id, public_id, name, code, description, active
       FROM resource_groups WHERE public_id = ?`,
    ).get(publicId) as TargetRow | undefined;
    if (!row) return undefined;
    const members = this.database.prepare(
      `SELECT r.public_id FROM resource_group_members gm
       JOIN resources r ON r.id = gm.resource_id
       WHERE gm.group_id = ? ORDER BY lower(r.name), r.public_id`,
    ).all(row.id) as { public_id: string }[];
  ...
  insertResource(input: { publicId: string; name: string; code: string | null; description: string; now: string }): CatalogTargetRecord {
    const result = this.database.prepare(
      `INSERT INTO resources (public_id, name, code, description, active, created_at, updated_at)
       VALUES (@publicId, @name, @code, @description, 1, @now, @now)`,
    ).run(input);
    const row = this.database.prepare(
      `SELECT id, public_id, name, code, description, active FROM resources WHERE id = ?`,
    ).get(Number(result.lastInsertRowid)) as TargetRow;
  ...
  insertGroup(input: { publicId: string; name: string; code: string | null; description: string; now: string }): CatalogGroupRecord {
    const result = this.database.prepare(
      `INSERT INTO resource_groups (public_id, name, code, description, active, created_at, updated_at)
       VALUES (@publicId, @name, @code, @description, 1, @now, @now)`,
    ).run(input);
    const row = this.database.prepare(
      `SELECT id, public_id, name, code, description, active FROM resource_groups WHERE id = ?`,
    ).get(Number(result.lastInsertRowid)) as TargetRow;
    return { ...mapTarget(row), memberResourceIds: [] };
  }

  updateResource(id: number, input: { name?: string; code?: string | null; description?: string; active?: boolean }, now: string): void {
    this.updateTarget("resources", id, input, now);
  }

  updateGroup(id: number, input: { name?: string; code?: string | null; description?: string; active?: boolean }, now: string): void {
    this.updateTarget("resource_groups", id, input, now);
  }
  ...
  replaceGroupMembers(groupId: number, resourceIds: number[], now: string): void {
    ...
    const insert = this.database.prepare(
      `INSERT INTO resource_group_members (group_id, resource_id, created_at) VALUES (?, ?, ?)`,
    );
  ...
  listAssignments(projectId: number): AssignmentRecord[] {
    ...
    ).all(projectId) as Array<{ id: number; public_id: string; project_id: number; task_id: number; task_public_id: string; kind: "resource" | "group"; target_internal_id: number; target_public_id: string }>;
    return rows.map((row) => ({
      id: row.id,
      publicId: row.public_id,
  ...
  replaceTaskAssignments(input: {
    projectId: number;
    taskId: number;
    targets: Array<{ publicId: string; kind: "resource" | "group"; internalId: number; assignmentPublicId: string }>;
    now: string;
  }): void {
    this.database.prepare(
      "DELETE FROM task_assignments WHERE project_id = ? AND task_id = ?",
    ).run(input.projectId, input.taskId);
    const insert = this.database.prepare(
      `INSERT INTO task_assignments
       (public_id, project_id, task_id, resource_id, group_id, created_at, updated_at)
       VALUES (@assignmentPublicId, @projectId, @taskId, @resourceId, @groupId, @now, @now)`,
    );
    for (const target of input.targets) {
      insert.run({
        assignmentPublicId: target.assignmentPublicId,
  ...
  insertAdminSession(input: { tokenHash: Buffer; createdAt: string; expiresAt: string }): number {
    const result = this.database.prepare(
      `INSERT INTO resource_catalog_admin_sessions (token_hash, created_at, expires_at)
  ...
  findAdminSessionByHash(tokenHash: Buffer): { id: number; expiresAt: string; revokedAt: string | null } | undefined {
    const row = this.database.prepare(
      `SELECT id, expires_at, revoked_at FROM resource_catalog_admin_sessions WHERE token_hash = ?`,
  ...
  revokeAdminSession(id: number, revokedAt: string): void {
    this.database.prepare(
      `UPDATE resource_catalog_admin_sessions SET revoked_at = COALESCE(revoked_at, ?) WHERE id = ?`,
    ).run(revokedAt, id);
  }
}
