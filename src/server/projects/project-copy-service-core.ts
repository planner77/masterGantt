...
import type { CopyProjectRequest, CopyProjectResponse, ProjectTaskDto } from "../../contracts/projects";
...
import { createSessionToken, sessionExpiry, type NewSessionToken } from "../security/session-core";
import { isCanonicalUuidV4 } from "./project-contract";
import { EditSessionInvalidError, PersistedScheduleInvalidError, RevisionMismatchError, recalculatePersistedHierarchy, type AuthorizedEditSession } from "./project-service-core";
...
function validSession(session: ReturnType<EditSessionRepository["findById"]>, project: ReturnType<ProjectRepository["findCredentialById"]>, authorization: AuthorizedEditSession, now: Date): boolean {
...
export class ProjectCopyService {
  ...
  constructor(private readonly database: Database.Database, options: ProjectCopyServiceOptions = {}) {
  ...
  async copy(authorization: AuthorizedEditSession, expectedRevision: number, input: CopyProjectRequest): Promise<CopiedProject> {
    ...
    for (let attempt = 0; attempt < ID_ATTEMPTS; attempt += 1) {
      ...
      if (!isCanonicalUuidV4(newPublicId) || this.projects.findByPublicId(newPublicId)) continue;
      ...
      const transaction = this.database.transaction(() => {
        ...
        if (!validSession(sourceSession, source, authorization, now)) throw new EditSessionInvalidError();
        if (!source || source.revision !== expectedRevision) throw new RevisionMismatchError();
        ...
        if (sourceTasks.length > MAX_PROJECT_TASKS) throw new PersistedScheduleInvalidError();
        const calendar = createWorkingCalendar({
          timezone: "Asia/Seoul",
          weekendDays: [6, 0],
          holidays: sourceHolidays.map((holiday) => ({ date: holiday.holidayDate, name: holiday.name })),
        });
        ...
        for (const holiday of sourceHolidays) this.schedules.insertHoliday(project.id, holiday.holidayDate, holiday.name, nowText);
        ...
        while (pending.length > 0) {
          ...
          for (let index = pending.length - 1; index >= 0; index -= 1) {
            ...
            if (sourceTask.parentId !== null && !newBySourceId.has(sourceTask.parentId)) continue;
            let taskPublicId = "";
            for (let idAttempt = 0; idAttempt < ID_ATTEMPTS; idAttempt += 1) {
              const candidate = this.generateTaskPublicId();
              if (isCanonicalUuidV4(candidate) && !this.schedules.taskPublicIdExists(candidate)) { taskPublicId = candidate; break; }
            }
            if (!taskPublicId) throw new Error("Unique task identifier could not be generated.");
            const inserted = this.schedules.insertTask({
              ...
              progress: input.resetProgress && sourceTask.type !== "summary" ? 0 : sourceTask.progress,
              description: sourceTask.description,
              url: sourceTask.url,
              parentId: sourceTask.parentId === null ? null : newBySourceId.get(sourceTask.parentId)!.id,
          ...
          if (insertedThisPass === 0) throw new PersistedScheduleInvalidError();
        }

        if (input.resetProgress) {
          const copied = this.schedules.listTasks(project.id);
          const derived = recalculateHierarchy(dtoTasks(copied), calendar);
        ...
        for (const sourceLink of sourceLinks) {
          ...
          for (let idAttempt = 0; idAttempt < ID_ATTEMPTS; idAttempt += 1) {
            ...
            if (isCanonicalUuidV4(candidate) && !this.schedules.linkPublicIdExists(candidate)) { linkPublicId = candidate; break; }
          }
          if (!linkPublicId) throw new Error("Unique link identifier could not be generated.");
        ...
        const externalById = new Map(copiedTasks.map((task) => [task.id, task.externalId]));
        return {
          response: {
            data: {
              project: {
                ...
                calendar: { timezone: "Asia/Seoul" as const, weekendDays: [6, 0] as [6, 0], holidays: sourceHolidays.map((holiday) => ({ date: holiday.holidayDate, name: holiday.name })) },
              },
              ...
              operation: { kind: "projectCopy" as const, sourcePublicId: source.publicId, sourceRevision: source.revision, counts: { tasks: copiedTasks.length, links: copiedLinks.length, holidays: sourceHolidays.length } },
              warnings: [] as [],
            },
          },
          rawSessionToken: newSession.rawToken,
        } satisfies CopiedProject;
      });
