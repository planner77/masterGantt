...
export type TaskEditorSaveResult =
...
export function prepareTaskEditorCommand(
  task: ProjectTaskDto,
  draft: TaskEditorDraft,
): { readonly command: ProjectTaskUpdateCommand | null; readonly error: string | null } {
  const invalid = (error: string) => ({ command: null, error });
  if (task.type === "summary") return invalid("요약 작업은 직접 수정할 수 없습니다.");

  const name = draft.name.trim();
  if (!isWellFormedUnicode(name) || Array.from(name).length < 1 || Array.from(name).length > 200) {
    return invalid("작업명은 1~200자의 올바른 문자열로 입력해 주세요.");
  }

  try {
    parseDateOnly(draft.start);
  } catch {
    return invalid("시작일은 1900-01-01~2199-12-31 범위의 올바른 날짜여야 합니다.");
  }
  ...
  if (
    ...
    (task.type === "milestone" ? duration !== 0 : duration < 1 || duration > MAX_TASK_DURATION)
  ) {
    return invalid(task.type === "milestone" ? "마일스톤의 기간은 0일입니다." : "기간은 1~10,000 사이의 정수 근무일로 입력해 주세요.");
  }

  const progress = Number(draft.progress);
  if (!draft.progress.trim() || !Number.isFinite(progress) || progress < 0 || progress > 100) {
    return invalid("진행률은 0~100 사이의 숫자로 입력해 주세요.");
  }

  if (!isWellFormedUnicode(draft.description) || Array.from(draft.description).length > 4000) {
    return invalid("설명은 4,000자 이하의 올바른 문자열이어야 합니다.");
  }

  const url = draft.url.trim();
  if (url) {
    if (!isWellFormedUnicode(url) || Array.from(url).length > 2048) return invalid("URL은 2,048자 이하여야 합니다.");
    try {
      const parsed = new URL(url);
      if (parsed.protocol !== "http:" && parsed.protocol !== "https:") return invalid("URL은 http 또는 https 주소여야 합니다.");
    } catch {
      return invalid("올바른 URL을 입력해 주세요.");
    }
  }
  ...
  const payload: ProjectTaskUpdatePayload = {
    ...
    ...(draft.description !== (task.description ?? "") ? { description: draft.description || null } : {}),
    ...(url !== (task.url ?? "") ? { url: url || null } : {}),
  };

  return {
    command: Object.keys(payload).length ? { taskId: task.taskId, payload } : null,
    error: null,
  };
}
