...
function isAssignedResponse(value: unknown): value is AssignedTargetsResponse {
  ...
  return !!data && typeof data === "object" && "projectRevision" in data && typeof data.projectRevision === "number" &&
    "catalogRevision" in data && typeof data.catalogRevision === "number" &&
    "assignments" in data && Array.isArray(data.assignments) && "targets" in data && Array.isArray(data.targets);
}

function isTargetsResponse(value: unknown): value is AssignmentTargetsResponse {
  if (!value || typeof value !== "object" || !("data" in value)) return false;
  const data = value.data;
  return !!data && typeof data === "object" && "catalogRevision" in data && typeof data.catalogRevision === "number" &&
    "targets" in data && Array.isArray(data.targets);
}

function targetKey(target: Pick<AssignmentTargetDto, "kind" | "id">): string {
...
export function TaskAssignmentEditor({ taskId, revision, editable, disabled, onApplied }: Props) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        if (editable) {
          const candidatesResponse = await fetch(`/api/projects/${encodeURIComponent(publicId)}/assignment-targets`, {
            credentials: "same-origin", cache: "no-store",
          });
        ...
        if (alive) setError("할당 정보를 불러오지 못했습니다. 편집 권한과 네트워크 상태를 확인해 주세요.");
      } finally {
  ...
  async function save() {
    ...
    try {
      const requested = [...selected].map((key) => {
        const separator = key.indexOf(":");
        return { kind: key.slice(0, separator) as "resource" | "group", id: key.slice(separator + 1) };
      });
      ...
      if (response.status === 401) {
        setError("편집 권한이 만료되었습니다. 다시 잠금을 해제해 주세요.");
        return;
      }
      if (!response.ok) {
        setError("할당을 저장하지 못했습니다. 비활성 대상 또는 중복 선택 여부를 확인해 주세요.");
        return;
      }
      await onApplied();
    } catch {
      setError("할당 저장 결과를 확인할 수 없습니다. 최신 정보를 다시 확인해 주세요.");
    } finally {
      setSaving(false);
    }
  }
  ...
  return <section className={styles.relations} aria-labelledby="task-assignment-title">
    ...
    {!loading && targets.length > 0 ? <div>
      {targets.map((target) => {
        const key = targetKey(target);
        const checked = selected.has(key);
        if (!editable && !checked) return null;
        return <label key={key} style={{ display: "flex", gap: ".5rem", alignItems: "center", margin: ".35rem 0" }}>
    ...
    <p className={styles.caption}>그룹 할당은 담당 팀 참조이며 구성원을 개인 할당으로 자동 복제하지 않습니다.</p>
    {editable ? <div className={styles.actions}>
      ...
      {disabled ? <span className={styles.caption}>작업 필드 변경을 먼저 저장하거나 취소한 뒤 할당을 변경하세요.</span> : null}
    </div> : null}
  </section>;
}
