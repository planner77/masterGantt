...
| `RESOURCE_CATALOG_ADMIN_PASSWORD` | Compose: 예 | 글로벌 Resource/Resource Group 관리자 비밀번호. 최초 seed는 1~12자이며 DB 런타임 변경값이 우선하고 Compose는 미설정 시 config 단계에서 fail-fast; 직접 실행 환경은 기존 API fail-closed 정책 유지 |
