...
| #118 | 모바일/태블릿 도구 모음 밀도와 정보 버튼 개선 | 이전 PR #146 head의 e2e 86 PASS/1 FAIL은 당시 기록. 최종 head `80159c5a…`의 [PR CI](https://github.com/planner77/masterGantt/actions/runs/35987961438) quality/e2e/docker PASS 뒤 10:49 UTC main `6386db86…`로 외부 병합됐다. [main CI](https://github.com/planner77/masterGantt/actions/runs/35989417971)는 10:52 UTC quality PASS·e2e/docker 진행 중, Issue open. 구현 전후 높이 실측 NOT TESTED |
...
| #140 | Grid Task/Summary/Milestone 이름 인라인 편집 | [PR #161](https://github.com/planner77/masterGantt/pull/161) 첫 head `91ed250d`의 [run 35985509853](https://github.com/planner77/masterGantt/actions/runs/35985509853)은 quality/typecheck `TS2322` FAIL. 한 줄 type REWORK 뒤 새 head `c38b1d01138013092558e2c8bf76a5c68206c765`의 [run 35986191642](https://github.com/planner77/masterGantt/actions/runs/35986191642)은 quality/docker PASS, e2e 103 PASS/17 FAIL(#140 전용 3건 포함). 8305 preview는 새 head healthy/readiness200; 원격 E2E 실패는 미수정/미완료 |
| #142 | Chart 날짜 셀 범위·Milestone 중앙 정렬 | [PR #162](https://github.com/planner77/masterGantt/pull/162) head `37d4a38a327abf360228630a99a589cbbfd0d6d2`의 [run 35988517048](https://github.com/planner77/masterGantt/actions/runs/35988517048) 시작. 10:47 UTC quality/docker PASS·e2e 진행 중. Core의 per-milestone 좌표 hook 부재로 사용자 결정에 따라 기존 drag/link를 보존하고 중앙 정렬 미충족을 명시. Task/Summary 좌표 E2E·문서 4파일만 추가, 제품 위치 변경 없음, version `0.32.0` 유지. 로컬 테스트 NOT TESTED, 전체 CI 결과 미확정, Issue open |
...
[PR #143](https://github.com/planner77/masterGantt/pull/143)은 현재 head `3de7b80c1aa4e0de1c39c789e189042dc44f2baf`의 [run 35975872207](https://github.com/planner77/masterGantt/actions/runs/35975872207)에서 quality/e2e/docker PASS를 받았다. 다만 그 문서 head는 현재 main `6386db86…`보다 오래된 상태라 이 최신 계획의 원격 반영 증거가 아니다. 해당 PASS를 #116 이후 제품 수정의 테스트/CI 근거로 재사용하지 않는다. #115는 아래 정식 릴리스·안전한 branch 정리 증거에 따라 종료됐고, #120의 별도 릴리스 범위·승인 판단은 여전히 대기한다.
...
- 기준/의존성: #140 [PR #161](https://github.com/planner77/masterGantt/pull/161) 최신 head `c38b1d01138013092558e2c8bf76a5c68206c765` / version `0.32.0`을 branch base로 한다. 첫 head의 `TS2322` 실패를 새 head의 PASS로 전용하지 않는다. 사용자는 주 보기에서 작업을 주 전체 폭으로 늘리지 않고 **주 셀 안의 실제 날짜 위치 유지**를 선택했다. 같은 날짜의 day segment를 week cell 내부에서 찾아 비교한다.
...
- 테스트/다음 handoff: 동일 fixture에서 Summary·다일 Task·1일 Task의 Day/Week 좌표를 ±1 CSS px(브라우저 rounding 허용)로 비교한다. Milestone 중앙 좌표는 명시적 pending/fixme로 남기고 PASS로 세지 않는다. header 실제 cell/scale width와 bar bounding box를 사용하며 week는 ISO 주 번호 문자열 대신 Core timeline의 실제 주 시작·7개 calendar day 경계(월·년·DST 전환 포함)에서 요일 위치를 계산한다. 제품에는 별도 zoom 조작이 없어 지원 scale 버튼 Day/Week를 검증하고 임의 zoom UI는 N/A다. scale 전환, horizontal scroll, viewport 390/768/1024/1440, progress/link/selection/readonly, editable drag/resize 뒤 canonical/revision 및 기존 Grid/Gantt 상태 보존은 구현된 assertion과 남은 계획을 분리한다. 사용자 지시에 따라 로컬 test/lint/typecheck/build/browser는 NOT TESTED다. 독립 정적 QA/DOCUMENTATION_SYNC를 마친 PR #162의 정확한 head `37d4a38a327abf360228630a99a589cbbfd0d6d2`에서 [CI run 35988517048](https://github.com/planner77/masterGantt/actions/runs/35988517048)이 시작됐다. 완료 결과와 미단언·미실행 항목을 PASS로 과대 표시하지 않는다. #140 PR #161의 E2E FAIL 103 PASS/17 FAIL은 선행 위험으로 유지한다.
