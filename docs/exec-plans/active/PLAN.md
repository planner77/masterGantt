...
#130 Phase 1 [PR #150](https://github.com/planner77/masterGantt/pull/150)의 최신 head `4d37d5cacbc24147236b7fd53dad7eb16c53f808`은 이전 head `e4e7a7fd…`에 현 main `f467478f…`의 #121 helper workflow 한 파일만 통합했다. [CI run 36011749093](https://github.com/planner77/masterGantt/actions/runs/36011749093)의 생성만 확인했으며 job 상태·완료 결과는 사용자 지시에 따라 조회하지 않았다. 이전 head `e4e7a7fd…`의 [run 36008880109](https://github.com/planner77/masterGantt/actions/runs/36008880109)은 당시 base `bbd288b3…`와의 병합에서 quality/e2e/docker PASS·E2E 103 PASS였고, 현재 main 조합의 PASS로 전용하지 않는다. [8309 Docker 미리보기](http://localhost:8309)는 이전 e4e head의 제품 코드로 healthy/readiness 200이며, 새 head는 workflow만 추가했으므로 제품 runtime blob은 동일하나 새 head 명의의 이미지는 만들지 않았다. 구현 전후 화면 실측은 NOT TESTED다. 후속 PR #152–#154는 마지막 조회에서 open/head 불변이고 새 수정이 자동 상속되지 않았다. 병합·Issue 종료는 진행하지 않았다.

2026-09-24 14:02 UTC 기준 #140 [PR #161](https://github.com/planner77/masterGantt/pull/161)의 head `22435cc115571b1f9494f8b93a5a90e88942c2b3` [CI run 36006366745](https://github.com/planner77/masterGantt/actions/runs/36006366745)은 quality/docker PASS·e2e 104 PASS/16 FAIL이며 #140 전용 3건은 모두 PASS다. #142 [PR #162](https://github.com/planner77/masterGantt/pull/162)의 head `51f7aa0108f62ff19832578b1f6afa6ec635e195` [CI run 36006864375](https://github.com/planner77/masterGantt/actions/runs/36006864375)은 quality/docker PASS·e2e 106 PASS/15 FAIL/1 skipped다. #142 Task/Summary Day/Week geometry 전용 테스트와 상속된 #140 전용 3건은 PASS이고, Milestone 중앙 정렬은 fixme skipped·AC 미충족이다. 두 PR의 required 전체 e2e는 FAIL이므로 최종 QA·병합은 보류한다. 순수 Tab→F2 경로는 별도 NOT TESTED다.

이전 #140 [PR #161](https://github.com/planner77/masterGantt/pull/161)의 head `c09f90f6943530c608fa42d24e92a2eefc23ee89` [CI run 35997955291](https://github.com/planner77/masterGantt/actions/runs/35997955291)은 quality/docker PASS·e2e 102 PASS/18 FAIL이다. #140 전용 2 PASS/1 FAIL이며 실패 trace에서 테스트의 DOM `.focus()` 뒤 F2가 의도한 leaf 대신 이전 Milestone을 편집해 PATCH taskId 순서가 1,3,4,4가 됨을 확인했다. 실제 키보드 경로와 Core 논리 focusCell/열린 editor 대상 검증이 REWORK다. 이를 선행으로 보존한 #142 [PR #162](https://github.com/planner77/masterGantt/pull/162) head `8d9f44e9c5a070d3a68e6eec5f49007d56f20896` [CI run 35998150981](https://github.com/planner77/masterGantt/actions/runs/35998150981)은 quality/docker PASS·e2e 104 PASS/17 FAIL/1 skipped다. #142 Task/Summary Day/Week geometry 전용 테스트는 PASS, Milestone 중앙 정렬은 fixme skipped·미충족이다. 두 PR의 required 전체 e2e와 최종 QA는 FAIL/REWORK이며 Issue는 열려 있다.

2026-09-24 12:24 UTC 조회 시 원격 main은 외부 #119 제품·문서 병합 및 lifecycle helper 추가로 `88eb67413c7b1d4b7a96cca3db98984810ed7839` / version `0.27.5`가 됐다. [main CI 35997721645](https://github.com/planner77/masterGantt/actions/runs/35997721645)는 quality/e2e/docker와 임시 image 게시·검증·정리 job이 PASS이며 #119 Issue는 open이다. 앞선 main `5699e051…`의 [CI 35994252173](https://github.com/planner77/masterGantt/actions/runs/35994252173)는 quality/e2e/docker와 임시 image 게시·검증·정리 job이 PASS였으나 현재 main의 결과로 전용하지 않는다. #116·#117·#118은 다른 작업에서 종료됐다. 이 작업에서는 병합·Issue 종료를 실행하지 않았다. #142 이전 D head의 별도 Docker 미리보기(8308)는 healthy/readiness 200이며 기존 미리보기와 volume은 보존했다. 아래 과거 main·Issue 상태는 당시 기록으로 읽고 최신 상태는 [UI/UX 실행 계획](UI_UX_ROLLOUT.md)을 따른다.
문서 [PR #165](https://github.com/planner77/masterGantt/pull/165)의 head `8e0780e1bea3cc4bf2e7ea73750808841a5d3c2c` [CI run 36007728414](https://github.com/planner77/masterGantt/actions/runs/36007728414)은 quality/e2e/docker PASS, E2E 102건 PASS다. 이 문서에 #130 최신 head·Docker 결과와 #140·#142 CI 결과를 추가한 변경은 해당 head 뒤의 미검증 수정이므로 이 PASS를 전용하지 않는다. 이전 문서 head `e069e9a7…`도 전체 PASS였고, 그 전 `96a7e7f…`의 API GET `ECONNRESET`에 따른 E2E FAIL은 당시 증거로 보존한다.
문서 PR #165의 이후 head `7efa4ed46e9c12ff71123d662fe0cab6645d4a5f`에서 [CI run 36010584132](https://github.com/planner77/masterGantt/actions/runs/36010584132)가 생성됐다. 사용자 최신 지시 이후 결과는 조회하지 않았고, 이 계획의 #130 최신 head 기록은 그 head 뒤의 문서 수정이므로 해당 run의 검증으로 전용하지 않는다.

원격 상태와 사용자 요청에 따른 최신 작업은 [UI/UX 실행 계획](UI_UX_ROLLOUT.md)을 따른다. #30을 제외하고 #115 → #116 → #117 → #118 → #119 → #121 및 #122 검증, 이어 #130의 Project List → Workspace → Task Editor → Search/Filter 단계는 각 PR/CI 시작까지 진행했으며 미완 검증·충돌은 실행 계획에 남긴다. #155 전체 화면 PR/CI 시작 뒤 새로 확인한 UI/UX 이슈는 #136 브랜드 버전 → #141 파비콘/탭 제목 → #138 프로젝트 상태·필터 → #140 Grid 이름 인라인 편집 → #142 Chart 날짜 셀 정렬 순서로 처리한다. 이미 병합된 #120 token을 재사용하며 각 Issue는 수정·문서 동기화·PR 생성·해당 head의 CI 시작까지만 진행한 후 다음 Issue로 이동한다. 이 작업의 병합/main 검증·정식 릴리스·Issue 종료는 보류한다. 다만 2026-09-24 08:01 UTC `planner77` 계정의 PR #144 병합으로 main이 `e91bcff22a6e087b510d5706c2e71a75c47ad92f` / version `0.27.2`로 이동했고 main quality/e2e/docker 및 임시 GHCR exact digest smoke·cleanup이 PASS한 원격 사실은 별도 기록한다. 변경본 로컬 Docker는 #140 [PR #161](https://github.com/planner77/masterGantt/pull/161) 과거 후보를 별도 볼륨과 8305 포트에서 확인했다. #142 이전 D head는 별도 8308 미리보기에서 healthy/readiness 200, version 0.32.0을 확인했다. 새 E head는 #140 테스트 변경만 상속해 제품 runtime blob은 같지만, 새 head 명의의 Docker 미리보기는 실행하지 않았다. 기존 미리보기와 운영 데이터는 보존한다. 아래 Wxx/과거 이슈 기록은 당시 상태이며 이번 변경의 검증 근거로 재사용하지 않는다.
