...
현재 #140 [PR #161](https://github.com/planner77/masterGantt/pull/161)의 editor DOM 재수정 head `5cdbe08afa9e06ef10b66b7ab3e6f82cc638a5cf`에서 [CI run 35994849520](https://github.com/planner77/masterGantt/actions/runs/35994849520), 이를 선행으로 보존한 #142 [PR #162](https://github.com/planner77/masterGantt/pull/162)의 head `9bf7e3ed919627dbdd2494073102e7404d616ee3`에서 [CI run 35995330434](https://github.com/planner77/masterGantt/actions/runs/35995330434) 시작을 확인했다. 11:58 UTC 두 최신 run 모두 quality/docker PASS·e2e 진행 중으로 전체 결과는 미판정이다. 직전 #140 run 35991431451은 quality/docker PASS·e2e 101 PASS/19 FAIL, #142 run 35992412304는 quality/docker PASS·e2e 102 PASS/19 FAIL/1 skipped였다. 실패 trace에 따라 #140 editor cell 교체와 #142 좁은 화면의 가상화된 주 셀 조회를 수정했다. #142 Milestone 중앙 정렬은 미충족이며 Issue는 열려 있다.

2026-09-24 11:58 UTC 조회 시 원격 main은 다른 작업자의 PR #143 문서 병합 등을 거쳐 `5699e0517836d796b05b0df27b6c2a5b6b2b553b` / version `0.27.4`가 됐다. [main CI 35994252173](https://github.com/planner77/masterGantt/actions/runs/35994252173)의 quality/e2e/docker와 임시 image 게시·검증·정리 job은 PASS다. #116·#117·#118은 다른 작업에서 종료됐다. 이 작업에서는 병합·Issue 종료를 실행하지 않았다. #142 최신 head의 별도 Docker 미리보기(8307)는 healthy/readiness 200이며 기존 미리보기와 volume은 보존했다. 아래 과거 main·Issue 상태는 당시 기록으로 읽고 최신 상태는 [UI/UX 실행 계획](UI_UX_ROLLOUT.md)을 따른다.

원격 상태와 사용자 요청에 따른 최신 작업은 [UI/UX 실행 계획](UI_UX_ROLLOUT.md)을 따른다. #30을 제외하고 #115 → #116 → #117 → #118 → #119 → #121 및 #122 검증, 이어 #130의 Project List → Workspace → Task Editor → Search/Filter 단계는 각 PR/CI 시작까지 진행했으며 미완 검증·충돌은 실행 계획에 남긴다. #155 전체 화면 PR/CI 시작 뒤 새로 확인한 UI/UX 이슈는 #136 브랜드 버전 → #141 파비콘/탭 제목 → #138 프로젝트 상태·필터 → #140 Grid 이름 인라인 편집 → #142 Chart 날짜 셀 정렬 순서로 처리한다. 이미 병합된 #120 token을 재사용하며 각 Issue는 수정·문서 동기화·PR 생성·해당 head의 CI 시작까지만 진행한 후 다음 Issue로 이동한다. 이 작업의 병합/main 검증·정식 릴리스·Issue 종료는 보류한다. 다만 2026-09-24 08:01 UTC `planner77` 계정의 PR #144 병합으로 main이 `e91bcff22a6e087b510d5706c2e71a75c47ad92f` / version `0.27.2`로 이동했고 main quality/e2e/docker 및 임시 GHCR exact digest smoke·cleanup이 PASS한 원격 사실은 별도 기록한다. 변경본 로컬 Docker는 #140 [PR #161](https://github.com/planner77/masterGantt/pull/161) 과거 후보를 별도 볼륨과 8305 포트에서 확인했다. #142 최신 head는 별도 8307 미리보기에서 healthy/readiness 200, version 0.32.0을 확인했다. 기존 미리보기와 운영 데이터는 보존한다. 아래 Wxx/과거 이슈 기록은 당시 상태이며 이번 변경의 검증 근거로 재사용하지 않는다.
