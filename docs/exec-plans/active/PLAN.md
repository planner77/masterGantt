...
- W22 진행 상태: 성공한 `main` commit용 immutable `ci-<full SHA>`와 SemVer release용 `sha-<full SHA>`를 분리하고, 두 게시 경로 모두 registry digest 재-pull 뒤 HTTP Project 생성·edit session·무인증 Task 거부·인증 Task 저장·restart 재조회를 검증하도록 구현 중이다. BuildKit SBOM/provenance는 항상 생성하고 GitHub Artifact Attestation은 지원 plan에서 `ENABLE_GITHUB_ATTESTATIONS=true`로 opt-in한다. Admin 인증과 private visibility는 확인됐으나 원격 workflow/package/digest 증거는 아직 NOT TESTED다. [진행 기록](../../W22_REVIEW.md)
