...
test.describe("Issue #4/#22 작업 메뉴와 보호된 편집기", () => {
  ...
  test("sends one explicit PATCH and uses canonical working-day results across weekends and holidays", async ({ page }) => {
    ...
    await editor(page).getByLabel("진행률", { exact: true }).fill("35");
    await save(page).click();
    await expect(editor(page)).toHaveCount(0);
    expect(fixture.patches[2].postDataJSON()).toEqual({ name: "Edited together", start: "2026-09-18", duration: 3, progress: 35 });
