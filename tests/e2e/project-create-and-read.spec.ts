...
test("생성·목록·직접 읽기·실제 링크 복사와 매번 비밀번호를 확인하는 삭제", async ({ browser, page }) => {
  ...
  await create.click();
  await expect(page.locator(".form-error")).toContainText("프로젝트 이름을 입력해 주세요.");
  await expect(page.getByLabel("프로젝트 이름")).toHaveAttribute("aria-invalid", "true");
  await expect(page.locator("#project-name-error")).toHaveText("프로젝트 이름을 입력해 주세요.");
  await page.getByLabel("프로젝트 이름").fill(name);
  await create.click();
  await expect(page.locator(".form-error")).toContainText("소유자를 입력해 주세요.");
  await expect(page.getByLabel("소유자")).toHaveAttribute("aria-describedby", /project-owner-error/);
  ...
  await create.click();
  await expect(page.locator(".form-error")).toContainText("편집 비밀번호는 1~12자로 입력해 주세요.");
  await expect(passwordInput).toHaveAttribute("aria-invalid", "true");
  await expect(page.locator("#project-password-error")).toHaveText("편집 비밀번호는 1~12자로 입력해 주세요.");
