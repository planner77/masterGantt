...
function createHarness(options: HarnessOptions = {}) {
  ...
  const dependencies: ResourceHandlerDependencies = {
    ...
    adminAuthRateLimiter: { consume: () => ({ allowed: true, retryAfterSeconds: 0 }) },
    ...options,
  };
...
describe("resource catalog administrator authentication diagnostics", () => {
  ...
  it("distinguishes a configured password in the 13 to 15 character bootstrap gap", async () => {
  ...
  it("rate limits administrator authentication before password KDF work", async () => {
    const unlockAdmin = vi.fn(() => undefined);
    const { dependencies, entries } = createHarness({
      unlockAdmin,
      adminAuthRateLimiter: { consume: () => ({ allowed: false, retryAfterSeconds: 37 }) },
    });
    ...
    expect(unlockAdmin).not.toHaveBeenCalled();
    expect(failure(entries)).toMatchObject({ reason_code: "RATE_LIMITED", status: 429 });
  });
