...
| `RESOURCE_CATALOG_ADMIN_PASSWORD` | 글로벌 Resource/Resource Group 관리 전용 비밀번호. Project 편집 비밀번호와 별도이며 최초 seed는 1~12자이며, UI에서 변경한 DB 자격증명이 이후 우선 적용. Docker Compose에서는 필수 입력이며 누락 시 `docker compose ... config` 단계에서 배포를 중단 |
